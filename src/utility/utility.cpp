...
Eigen::Matrix4f Pose2Matrix(pose_type pose){
    ...
    float s_r = sinf(pose.orient(0));
    float c_r = cosf(pose.orient(0));
    float s_p = sinf(pose.orient(1));
    float c_p = cosf(pose.orient(1));
    float s_y = sinf(pose.orient(2));
    float c_y = cosf(pose.orient(2));

    Eigen::Matrix3f R_x;    // 计算旋转矩阵的X分量
    R_x <<  1,  0,    0,
            0,  c_r, -s_r,
            0,  s_r,  c_r;

    Eigen::Matrix3f R_y;    // 计算旋转矩阵的Y分量
    R_y << c_p, 0,  s_p,
           0,   1,  0,
          -s_p, 0,  c_p;

    Eigen::Matrix3f R_z;    // 计算旋转矩阵的Z分量
    R_z << c_y, -s_y, 0,
           s_y,  c_y, 0,
            0,   0,   1;
