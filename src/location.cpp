...
public:
    ...
    ros::Publisher odom_pub_; //发布当前里程计
    ...
    location(ros::NodeHandle nh, ros::NodeHandle nh_private){
        ...
        std::string pub_odom_topic = config["pub_odom_topic"].as<std::string>();
        odom_pub_ = nh_.advertise<nav_msgs::Odometry>(pub_odom_topic, 10, true);
    ...
    void publish_pose(pose_type pose){
        ...
        odom_msg.header.stamp = pose_stamped.header.stamp;
        //odom_msg.child_frame_id = odom_child_frame_id_;
        odom_msg.pose.pose = pose_stamped.pose;

        Eigen::Matrix<double, 6, 1> vel = eskf->getVel();
        double cth = cos(pose.orient(2));
        double sth = sin(pose.orient(2));

        double vx = cth * vel(0) + sth * vel(1);
        double vy = -sth * vel(0) + cth * vel(1);
        double vz = vel(2);

        if(std::abs(vx) > 15.0) vx = 0.0;
        if(std::abs(vy) > 15.0) vy = 0.0;
        if(std::abs(vz) > 15.0) vz = 0.0;
        ...
        odom_msg.twist.twist.angular.x = vel(3);
        odom_msg.twist.twist.angular.y = vel(4);
        odom_msg.twist.twist.angular.z = vel(5);
        ...
        while(ros::ok()){
            if(rece_flag){//接收到点云
                ...
                if(req_reloc_){ //重定位
                    ...
                    auto start_reloc = std::chrono::system_clock::now();

                    localizier3d->setRelocMode();
                    act_pose = localizier3d->location(act_pose, cloud_in);
    
                    std::cout << " Loc :  relocation finish: "<< act_pose.pos(0) 
                    << " ; " << act_pose.pos(1) <<" ; " << act_pose.orient(2) << ";" <<std::endl;
                    ...
                    auto end_reloc = std::chrono::system_clock::now();
                    std::cout << " Loc :  relocation duration: "<< std::chrono::duration_cast<std::chrono::milliseconds>(end_reloc - start_reloc).count() << "ms" << std::endl;
                    ...
                    //点云在世界坐标系下的数据
                    //cloud_world = cloud_transform(cloud_in, world_pose_matrix);
...
int main(int argc, char** argv){
    ros::init(argc, argv, "lidar_location_3d");

    ROS_INFO("\033[1;32m---->\033[0m Lidar Location 3d Started.");
